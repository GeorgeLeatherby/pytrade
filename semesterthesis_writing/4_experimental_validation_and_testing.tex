...
Two results in Table~\ref{tab:saa-per-asset-metrics} are worth flagging as specifically problematic. The first is val\_02 taken as a whole: a period-average Asset $\alpha$ of essentially zero ($0.04\%$) alongside a clearly negative Market $\alpha$ ($-9.06\%$) and negative risk-adjusted ratios shows the checkpoint neither beating its own assets' passive baselines nor the broader market, during exactly the kind of correlated shock Section~\ref{sec:saa}'s single-asset design is least equipped to handle. The second is Crude in the same block: a maximum drawdown of $-37.85\%$, by far the largest in the table, coincides with a positive final Asset $\alpha$ of $+8.31\%$. A favorable terminal alpha figure can therefore still conceal a severe intra-episode drawdown, which is why the drawdown-aware reward terms of Section~\ref{sec:drawdown} and the Calmar ratio reported alongside raw returns are necessary complements to reading this table by final alpha alone.
